...
function möb(a, b, c) {
  ...
  var pb = b[0] * b[0] + b[1] * b[1];
  var pc = c[0] * c[0] + c[1] * c[1];

  var y = ((a[0] - b[0]) * (pb - pc) - (b[0] - c[0]) * (pa - pb)) / (2 * (b[1] - a[1]) * (b[0] - c[0]) - 2 * (a[0] - b[0]) * (c[1] - b[1]));

  var x = (pa - pb + 2 * (b[1] - a[1]) * y) / (2 * (a[0] - b[0]));
  var r = Math.sqrt((x - a[0]) * (x - a[0]) + (y - a[1]) * (y - a[1]));
  return [x, y, r];
}
...
function spiral(r, st, delta, opts) {
  var rd = crecip(delta);
  var md = modulus(delta);
  var mrd = 1 / md;
  var colidx = opts.i;
  var cols = opts.fill;
  var min_d = opts.min_d;
  var max_d = opts.max_d;
