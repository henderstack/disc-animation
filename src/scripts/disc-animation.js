...
function möb(a, b, c) {
  const pa = a[0] * a[0] + a[1] * a[1];
  const pb = b[0] * b[0] + b[1] * b[1];
  const pc = c[0] * c[0] + c[1] * c[1];
  ...
  const x = (pa - pb + 2 * (b[1] - a[1]) * y) / (2 * (a[0] - b[0]));
  const r = Math.sqrt((x - a[0]) * (x - a[0]) + (y - a[1]) * (y - a[1]));
  return [x, y, r];
}

function arcs(x, y, r) {
  const c = möb_arc(x, y, r);
  if (c[2] > 10) return;
  $.beginPath();
...
function möb_pt(x, y) {
  const denom = (x + 1) * (x + 1) + y * y;
  return [(x * x - 1 + y * y) / denom, 2 * y / denom];
}
function möb_arc(x, y, r) {
  const a = möb_pt(x - r, y);
  const b = möb_pt(x + r, y);
  const c = möb_pt(x, y + r);
  return möb(a, b, c);
}
...
function spiral(r, st, delta, opts) {
  const rd = crecip(delta);
  const md = modulus(delta);
  const mrd = 1 / md;
  let colidx = opts.i;
  const cols = opts.fill;
  const min_d = opts.min_d;
  const max_d = opts.max_d;
...
function anim(t) {
  ...
  let beg = rotate(root.a, Math.PI * 2 * t);
  for (let i = 0; i < q; i++) {
...
console.log("Animation should be running - Henderstack");


// ======= ES5 Version, Deprecated, but not removed, for example purposes ======= 
